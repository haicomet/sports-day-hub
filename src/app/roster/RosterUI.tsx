                  ...
                  <tr key={player.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        ...
                        {player.role === "admin" && <span className="text-[10px] bg-purple-100 text-purple-800 px-2 py-0.5 rounded-full border border-purple-200 uppercase tracking-wider">Admin</span>}
                      </div>
                    </td>
                      ...
                      {editingId === player.id ? (
                        <div className="flex items-center gap-2">
                          <input 
                            ...
                            placeholder="A.K.A..."
                            className="border border-slate-300 dark:border-slate-600 rounded-lg p-1.5 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 w-24 md:w-32"
                            maxLength={20}
                            autoFocus
                          />
                          <button onClick={() => handleSaveNickname(player.id)} disabled={isProcessing} className="text-emerald-600 hover:text-emerald-700 font-bold text-sm bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded">Save</button>
                          <button onClick={() => setEditingId(null)} className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 font-bold text-sm bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">Cancel</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-3">
                          ...
                          {canEditNickname && (
                            <button 
                              ...
                              className="text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 w-8 h-8 rounded-full flex items-center justify-center"
                              title="Edit Nickname"
                            >
                    ...
                    <td className="p-4">
                      {assignedTeam ? (
                        <span className="font-bold text-xs md:text-sm px-3 py-1 rounded-full border bg-opacity-10 dark:bg-opacity-20 whitespace-nowrap" style={{ color: assignedTeam.color.toLowerCase(), borderColor: assignedTeam.color.toLowerCase(), backgroundColor: assignedTeam.color.toLowerCase() }}>
                          {assignedTeam.name}
                        </span>
                      ) : (
                      ...
                      <td className="p-4 text-right">
                        {player.id !== currentUser.id ? (
                          <button 
                            ...
                            className="text-slate-400 hover:text-red-500 bg-slate-50 hover:bg-red-50 dark:bg-slate-800 dark:hover:bg-red-900/20 transition-colors p-2 rounded-lg inline-flex"
                            title="Delete Player"
                          >
